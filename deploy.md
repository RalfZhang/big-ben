...
5. 想单独调语气，不用等真有人回复：
   ...
   # 多给几句就是一条评论串，你我交替，最后一句是这次要回的（每句各自带引号）
   npm run threads:reply:try -- "你这钟准吗" "我走得比你上班还准时" "好吧"
   ```

   这个只调 AI，完全不碰 Threads。人格提示词就在 `reply.js` 的 `persona()` 里

**不是每条都回**：聊到对方只剩一句「好吧」「挺好」「谢谢」的时候就该收摊了，
再接一句就是尬聊。判断分两道：

- 本地先筛（`looksLikeClosing()`）—— 剥掉标点表情之后只剩「好吧」「行」「收到」
  这类词，且**这串里自己已经说过话**，直接闭嘴，连 AI 都不问。第一次有人来搭话不走这道：
  同样一句「哈哈」，接在几轮对话后面基本是收尾，出现在第一条评论里却未必，交给 AI 拿捏
- 本地拿不准的交给 AI：人格里那条规则让它觉得不用回时只输出 `__SKIP__`
  暗号（跟问时间那个 `__TIME__` 一个套路）。两头都是「值得接才接，别硬接」，
  但拿捏的地方不一样：已经聊过几轮的串看「对方是不是在收尾」，头一回搭话的看
  「这句有没有话头」——「打卡」「咣」这种两个字的也接，纯客套、纯表情、广告刷屏就不接

判成「不回」的评论会进 `seen`，下一轮不再重新问 AI —— 免费档额度经不起反复花。
日志里是 `reply: 不回 @某人 [聊完了/本地判] …`，每轮小结里也单列一栏「N 不用回」。

**上下文**：判断和编回复之前，会顺着 `replied_to` 把同一评论串里往前数的几条消息
（`config.threads.reply.contextMessages`，默认 4，0 = 关掉）一起喂给 AI，
不然它分不清这是第一次搭话还是聊了半天。爬到根帖就停，每条截到 120 字 ——
免费档模型喂太多反而跑偏。这些消息全部来自已经拉到手的那批会话数据，**不额外发请求**。
